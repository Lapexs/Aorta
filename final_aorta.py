...
mesh = pv.read(r"C:\Users\PC\Documents\GitHub\Aorta\Aorta\svprojects_0003_H_AO_SVD_Models_0065_1001.vtp")
...
#Slicer
slicer_fcsv_path = r"C:\Users\PC\Documents\GitHub\Aorta\Aorta\svprojects_0003_H_AO_SVD_Models_0065_1001_slicer.vtp"
slicer_centerline = pv.read(slicer_fcsv_path)
slicer_pts = slicer_centerline.points
...
plotter.add_mesh(slicer_pts, color = 'green', opacity=.7, point_size=6)

plotter.add_title("Smoothed Aortic Centerline")
plotter.add_legend()
plotter.show()

# Parametr tolerancji (w mm)
tolerance = 0.25

# Drzewo KD dla punktów Slicera
tree = cKDTree(slicer_pts)

# Dla każdego punktu z naszego algorytmu sprawdź, czy jest blisko jakiegoś punktu Slicera
matches = 0
for pt in smoothed_coords:
    dist, _ = tree.query(pt)
    if dist <= tolerance:
        matches += 1

coverage_percent = (matches / len(smoothed_coords)) * 100
print(f"{coverage_percent:.2f}% punktów z naszego algorytmu pokrywa się z centerline Slicera (tolerancja {tolerance} mm)")